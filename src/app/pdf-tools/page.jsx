...
function MergeTab() {
    ...
    return (
        <AnimatePresence mode="wait">
            {status === "idle" && (
                <motion.div key="merge-input" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-6">
                            ...
                            <CardContent className="pt-4 space-y-2">
                                {files.map((file, index) => (
                                    <div key={`${file.name}-${index}`} className="flex items-center justify-between gap-3 p-3 rounded-md bg-surface-hover/50 border border-surface-border/30">
                                        ...
                                        <button onClick={() => handleRemoveFile(index)} className="text-text-muted hover:text-error transition-colors shrink-0"><Trash2 size={14} /></button>
                                    </div>
                                ))}
            ...
            {status === "processing" && (
                <motion.div key="merge-proc" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <ProcessingStatus progress={progress} statusText="Merging PDFs..." />
                </motion.div>
            )}
            {status === "success" && result && (
                <motion.div key="merge-done" className="max-w-2xl mx-auto">
...
function SplitTab() {
    ...
    return (
        <AnimatePresence mode="wait">
            {status === "idle" && !file && (
                <motion.div key="split-drop" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                    <DropZone onFilesAccepted={handleFileAccepted} title="Upload PDF" description="Drag and drop a PDF file to split" accept={PDF_ACCEPT} />
                </motion.div>
            )}
            {(status === "idle" || status === "error") && file && (
                <motion.div key="split-cfg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-6">
                    <Card>
                        <CardHeader className="border-b border-surface-border/50 pb-4">
                            <CardTitle className="text-sm flex items-center justify-between">
                        ...
                        <CardContent className="pt-6 space-y-4">
                            ...
                            {ranges.map((range, index) => (
                                <div key={index} className="flex items-center gap-3">
                                    ...
                                    {ranges.length > 1 && <button onClick={() => handleRemoveRange(index)} className="text-text-muted hover:text-error transition-colors"><Trash2 size={14} /></button>}
                                </div>
                            ))}
            ...
            {status === "processing" && (
                <motion.div key="split-proc" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <ProcessingStatus progress={progress} statusText="Splitting PDF..." />
                </motion.div>
            )}
            {status === "success" && results.length > 0 && (
                <motion.div key="split-done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    ...
                    {results.map((r, i) => (
                        <Card key={i} className="p-4">
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleDownloadResult(r)}>Download</Button>
                            </div>
                        </Card>
                    ))}
                    <Button variant="ghost" onClick={handleReset} className="mt-4">Split Another PDF</Button>
                </motion.div>
            )}
        </AnimatePresence>
    )
...
function PdfToImageTab() {
    ...
    return (
        <AnimatePresence mode="wait">
            {status === "idle" && !file && (
                <motion.div key="p2i-drop" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                    <DropZone onFilesAccepted={(f) => setFile(f[0])} title="Upload PDF" description="Drag and drop a PDF to convert each page to an image" accept={PDF_ACCEPT} />
                </motion.div>
            )}

            {(status === "idle" || status === "error") && file && (
                <motion.div key="p2i-cfg" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <Card>
                        <CardHeader className="border-b border-surface-border/50 pb-4">
                            <CardTitle className="text-sm flex items-center justify-between">
                                <div className="flex items-center gap-2"><FileText size={14} className="text-accent" /><span className="font-mono">{file.name}</span></div>
                                <Button variant="ghost" size="sm" onClick={handleReset} className="text-xs">Change file</Button>
                            </CardTitle>
                        </CardHeader>
                        <CardContent className="pt-6 space-y-6">
                            <div className="space-y-3">
                                ...
                                <div className="flex gap-2">
                                    {["png", "jpeg"].map((f) => (
                                        <button key={f} onClick={() => setFormat(f)} className={cx(
            ...
            {status === "processing" && (
                <motion.div key="p2i-proc" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <ProcessingStatus progress={progress} statusText="Rendering pages to images..." />
                </motion.div>
            )}

            {status === "success" && results.length > 0 && (
                <motion.div key="p2i-done" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
                    ...
                    {results.map((r, i) => (
                        <Card key={i} className="p-4">
                            <div className="flex items-center justify-between">
                                ...
                                <Button variant="outline" size="sm" onClick={() => handleDownloadResult(r)}>
                                    <Download size={14} className="mr-1" /> Download
                                </Button>
                            </div>
                        </Card>
                    ))}
                    <Button variant="ghost" onClick={handleReset} className="mt-4">Convert Another PDF</Button>
                </motion.div>
            )}
        </AnimatePresence>
    )
